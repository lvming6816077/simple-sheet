...
const FloatImage = React.memo(observer((props: _FloatImage) => {
    ...
    const { image, width: imageWidth, height: imageHeight, status } = useImage({ imgUrl });
    ...
    return (
        <>
            <Image ref={imgRef} x={_x} y={_y} height={_height} width={_width} image={image} draggable onClick={onSelect} onTap={onSelect} onDragEnd={(e) => {
          onChange({
            x: e.target.x(),
            y: e.target.y(),
            width:width,
